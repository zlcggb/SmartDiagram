...
GHCR 不走 Docker Hub 的 `registry-mirrors`。国内环境因此还会把 `UV_IMAGE` 与 `UV_PYTHON_IMAGE` 默认切换到 `ghcr.1ms.run`，并在备份前预拉取；Dockerfile 通过 build args 消费这些地址。环境变量或根目录 `.env` 可以覆盖默认值，供私有镜像仓库使用。
