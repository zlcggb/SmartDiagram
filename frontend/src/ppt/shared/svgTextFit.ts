...
function textLines(inner: string) {
  ...
  for (const [index, span] of spans.entries()) {
    ...
    if (!value) continue;
    const explicitLineStart = index === 0 || numericAttr(attrs, "dy") !== null || numericAttr(attrs, "y") !== null || numericAttr(attrs, "x") !== null;
...
function wrapText(value: string, maxWidth: number, fontSize: number, maxLines: number) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (lines.length === maxLines - 1) {
      current += tokens.slice(index + 1).join("");
      break;
    }
  }
...
export function fitSvgTextToBounds(svg: string): SvgTextFitResult {
  ...
  const fitted = svg.replace(TEXT_TAG_RE, (full, attrs: string, inner: string) => {
    ...
    const hasOverflow = sourceLines.some((line) => estimatedTextWidth(line, fontSize) > dataW);
    ...
    const maxLines = Math.max(1, Math.floor((dataH + 0.5) / lineHeight));
    const lines = wrapText(plain, dataW, fontSize, maxLines);
    const nextAttrs = addAutoFitMarker(attrs);
    adjustedTextCount += 1;

    if (lines.length === 1) {
      return `<text${nextAttrs} textLength="${dataW}" lengthAdjust="spacingAndGlyphs">${escapeXmlText(lines[0] ?? "")}</text>`;
    }

    const tspans = lines
      .map((line, index) => {
        const needsCompression = estimatedTextWidth(line, fontSize) > dataW;
        const lengthAttrs = needsCompression ? ` textLength="${dataW}" lengthAdjust="spacingAndGlyphs"` : "";
        const dy = index === 0 ? 0 : Number(lineHeight.toFixed(2));
        return `<tspan x="${x}" dy="${dy}"${lengthAttrs}>${escapeXmlText(line)}</tspan>`;
      })
      .join("");
...
export function getSvgTextBoxIssues(svg: string) {
  ...
  for (const match of svg.matchAll(SVG_CONTENT_TAG_RE)) {
    ...
    if (dataW === null || dataH === null) {
      ...
      const fontSize = numericAttr(attrs, "font-size") ?? 18;
      if (textLines(inner).some((line) => estimatedTextWidth(line, fontSize) > dataW * 1.02)) {
