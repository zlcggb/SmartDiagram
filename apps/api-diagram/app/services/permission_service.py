...
def permission_context_from_identity(identity, body: dict[str, Any] | None = None) -> PermissionContext:
    """Build permission context from verified platform identity (never trust body auth fields)."""

    body = body or {}
    if identity.kind == "guest":
        from app.services.guest_session_service import guest_to_permission_context

        context: PermissionContext = guest_to_permission_context(str(identity.guest_id))
    else:
        from app.services.auth_service import user_to_permission_context

        context = user_to_permission_context(identity.user or {})
