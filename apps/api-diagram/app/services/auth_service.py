...
async def authenticate_db_user(email: str, password: str, session) -> dict[str, Any] | None:
    ...
    from app.services.account_profile_service import resolve_effective_scopes
...
async def register_db_user(email: str, password: str, session) -> dict[str, Any]:
    ...
    from app.services.platform_role_service import maybe_grant_bootstrap_admin_on_registration

    await maybe_grant_bootstrap_admin_on_registration(session, normalized_email, user_id)

    created_user = await session.get(User, user_id)
    from app.services.account_profile_service import resolve_effective_scopes

    scopes = resolve_effective_scopes(created_user) if created_user else MEMBER_SCOPES
