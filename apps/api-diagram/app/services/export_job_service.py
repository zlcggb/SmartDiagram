...
async def run_redis_export_jobs_once(
    *,
    limit: int = 10,
    queue_name: str | None = None,
) -> dict[str, Any]:
    """Process export jobs from a Redis list queue once."""

    queue = queue_name or settings.EXPORT_REDIS_QUEUE
    processed: list[dict[str, Any]] = []
    for _ in range(max(1, limit)):
        job_id = await dequeue_job(queue)
        if not job_id:
            break
        result = await process_export_job(job_id)
        processed.append(
            {
                **result,
                "queue_backend": "redis",
