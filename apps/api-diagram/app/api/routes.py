...
from app.services.user_quota_service import evaluate_user_budget
...
from app.services.permission_service import build_permission_context, permission_context_from_identity
from app.services.identity_service import require_request_identity
from app.services.guest_quota_service import consume_guest_ai_quota
from app.services.guest_quota_settings_service import evaluate_guest_budget, get_guest_quota_settings
from app.services.guest_usage_service import record_guest_ai_usage
...
async def chat_stream(request: Request):
    ...
    guest_quota_config: dict | None = None
    if identity.kind == "guest":
        client_ip = request.client.host if request.client else "unknown"
        try:
            async with async_session() as session:
                guest_quota_config = await get_guest_quota_settings(session)
                await session.commit()
        except Exception as exc:
            logger.warning(f"Guest quota settings load skipped: {exc}")
        quota = await consume_guest_ai_quota(
            str(identity.guest_id),
            client_ip,
            quota_settings=guest_quota_config,
        )
        if not quota.get("allowed"):
            raise HTTPException(
                ...
                detail={
                    ...
                    "hard_limit_enabled": quota.get("hard_limit_enabled", True),
                },
            )

    permission_context = permission_context_from_identity(identity, body)
    try:
    ...
    if identity.kind == "guest" and guest_quota_config:
        try:
            async with async_session() as session:
                guest_budget_guard = await evaluate_guest_budget(
                    session,
                    str(identity.guest_id),
                    runtime_guard,
                    guest_quota_config,
                )
                if not guest_budget_guard["allowed"]:
                    session.add(
                        AuditEvent(
                            tenant_id=permission_context.get("tenant_id") or "guest-pool",
                            project_id=permission_context.get("project_id"),
                            user_id=str(identity.guest_id),
                            event_type="guest.quota.rejected",
                            severity="warning",
                            message="Guest request rejected by token/cost quota.",
                            metadata_json=guest_budget_guard,
                        )
                    )
                    await session.commit()
                    raise HTTPException(status_code=402, detail=guest_budget_guard)
        except HTTPException:
            raise
        except Exception as exc:
            logger.warning(f"Guest token quota check skipped: {exc}")

    if identity.kind == "guest":
        try:
            async with async_session() as session:
                await record_guest_ai_usage(
                    session,
                    guest_id=str(identity.guest_id),
                    ip=request.client.host if request.client else "unknown",
                    estimated_total_tokens=int(runtime_guard.get("estimated_total_tokens") or 0),
                    estimated_cost=float(runtime_guard.get("estimated_cost") or 0.0),
                )
                await session.commit()
        except Exception as exc:
            logger.warning(f"Guest usage record skipped: {exc}")

    budget_guard = {
        "allowed": True,
        "reason": "guest_budget_skipped" if identity.kind == "guest" else "budget_check_unavailable",
        "period": "",
    ...
    if identity.kind != "guest":
        try:
            async with async_session() as session:
                ...
                user_id = str(permission_context.get("user_id") or "")
                if user_id:
                    from app.models.tenant import User

                    db_user = await session.get(User, user_id)
                    if db_user:
                        user_guard = await evaluate_user_budget(session, db_user, runtime_guard)
                        if not user_guard["allowed"]:
                            session.add(
                                AuditEvent(
                                    tenant_id=permission_context.get("tenant_id") or "local",
                                    project_id=permission_context.get("project_id"),
                                    user_id=user_id,
                                    event_type="user.quota.rejected",
                                    severity="warning",
                                    message="Request rejected by user quota profile.",
                                    metadata_json=user_guard,
                                )
                            )
                            await session.commit()
                            raise HTTPException(status_code=402, detail=user_guard)
                await session.commit()
        except HTTPException:
            raise
        except Exception as exc:
