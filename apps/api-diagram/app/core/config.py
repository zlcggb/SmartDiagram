...
_BACKEND_ROOT = Path(__file__).resolve().parents[2]  # apps/api-diagram
_REPOSITORY_ROOT = _BACKEND_ROOT.parent.parent  # monorepo root
# 统一配置在仓库根 .env；可选的 apps/api-diagram/.env 仅作本机覆盖。
...
class Settings:
    ...
    GUEST_SESSION_TTL_SECONDS: int = int(os.getenv("GUEST_SESSION_TTL_SECONDS", "86400"))
    GUEST_CAPTCHA_REQUIRED: bool = os.getenv("GUEST_CAPTCHA_REQUIRED", "false").lower() == "true"
    GUEST_AI_MAX_USES: int = int(os.getenv("GUEST_AI_MAX_USES", "5"))
    GUEST_AI_MAX_USES_PER_IP: int = int(os.getenv("GUEST_AI_MAX_USES_PER_IP", "20"))
    GUEST_AI_WINDOW_SECONDS: int = int(os.getenv("GUEST_AI_WINDOW_SECONDS", "86400"))
    API_ORIGIN_CHECK_ENABLED: bool = os.getenv("API_ORIGIN_CHECK_ENABLED", "false").lower() == "true"

    # Database（优先 DIAGRAM_DATABASE_URL，避免与 PPT 的 DATABASE_URL 冲突）
    DATABASE_URL: str = os.getenv("DIAGRAM_DATABASE_URL") or os.getenv(
        "DATABASE_URL",
    ...
    EXPORT_REDIS_QUEUE: str = os.getenv(
        "EXPORT_REDIS_QUEUE",
        "smartdiagram:queue:exports",
    )
