...
export async function resolvePptPrincipal(
  ...
  if (authorization) {
    ...
    if (payload.kind === "guest") {
      const guestId = String(payload.guest?.id ?? "").trim();
      if (!guestId) {
        throw new PptAuthError("统一认证服务返回了无效访客身份", 503);
      }
      return { kind: "guest", guestKey: hashGuestId(guestId) };
    }
  ...
  throw new PptAuthError("缺少有效的登录或访客会话", 401);
}

export function hashGuestId(guestId: string) {
  return crypto.createHash("sha256").update(`guest:${guestId}`).digest("hex");
}
