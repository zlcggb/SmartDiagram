...
import { clearGuestSession, ensureGuestSession } from '@/shared/lib/config/guestSession';
  ...
  bootstrapAuth: async () => {
    ...
    if (!session) {
      try {
        ...
        // Guest bootstrap may fail when API is down; modules will retry on demand.
      }
      set({ checking: false });
      return;
    }
