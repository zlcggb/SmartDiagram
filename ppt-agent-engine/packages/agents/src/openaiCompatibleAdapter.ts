...
export function parseOpenAiChatSseBuffer(buffer: string) {
  ...
  return { deltas, remaining: buffer.slice(cursor) };
}
...
export class OpenAiCompatibleAdapter implements GeminiAdapter {
  ...
  constructor() {
    ...
    this.designModel = resolveDesignModel();
  }
  ...
  private async chat(
    ...
    try {
      const response = await undiciFetch(chatCompletionsUrl(this.baseUrl), this.buildFetchOptions(body));
      if (!response.ok) {
        const errorText = await response.text();
        // 部分网关不支持 response_format，去掉后重试一次（仅非流式）
        if (options.json && !onToken && (response.status === 400 || response.status === 422)) {
          const retryBody = { ...body };
          delete retryBody.response_format;
          const retry = await undiciFetch(chatCompletionsUrl(this.baseUrl), this.buildFetchOptions(retryBody));
          if (!retry.ok) {
            throw new Error(explainApiError(retry.status, await retry.text()));
          }
          return contentFromChatPayload(await retry.json());
        }
        throw new Error(explainApiError(response.status, errorText));
      }

      if (onToken) {
        return await this.readStream(response, onToken);
      }
      return contentFromChatPayload(await response.json());
    } catch (error) {
