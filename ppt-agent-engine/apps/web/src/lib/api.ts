...
export function getApiBase() {
  const raw = import.meta.env?.VITE_API_BASE_URL ?? "http://127.0.0.1:4000";
...
function pickProvider(...sources: Array<Record<string, unknown> | null>): string | undefined {
  for (const source of sources) {
    if (source) {
      ...
      if (typeof source.provider === "string") return source.provider;
    }
  }
  return undefined;
}
  ...
  for (const candidate of candidates) {
    ...
    if (fromCounts !== null) {
      return {
        ...
        model: (typeof root.model === "string" ? root.model : undefined) || "gpt-5.6-sol",
        tokenUsage,
      };
    }
  }
...
async function requestWith<T>(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`请求失败（HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ""}）`);
    }
...
export const api = {
  ...
  async getAiUsageSummary(projectId?: string): Promise<AiUsageSummary | null> {
    ...
    const modelName = (typeof status?.model === "string" ? status.model : null) ||
                      (typeof status?.provider === "string" ? status.provider : "gpt-5.6-sol");
